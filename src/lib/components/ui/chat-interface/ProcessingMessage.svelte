		...
		return textSteps
			.map((step, index) => {
				...
				const formattedTitle = title.replace(
					/\*\*(.*?)\*\*/g,
					`<div class="font-semibold text-primary mb-1 ${shimmerClass}">$1</div>`
				);

				const formattedDescription = description
					? `<div class="text-muted-foreground text-xs leading-relaxed pl-2 border-l-2 border-primary/20">${description}</div>`
					: '';
...
{#if showMessage}
	<div class="flex justify-start mb-6" in:fade={{ duration: 400 }}>
		<div
			class="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950/30 dark:to-purple-950/30 rounded-xl p-6 max-w-2xl border border-blue-200/50 dark:border-blue-800/30 shadow-sm"
		>
			<div class="flex items-start gap-3">
				...
				>
					<Icon
						icon="lucide:zap"
						class="w-4 h-4 text-primary {isComplete ? '' : 'animate-pulse'}"
					/>
				</div>
					...
					{#if steps && steps.length > 0}
						{#each steps as step, index (step.title + index)}
							...
							<div class="mb-4">
								...
								{#if step.description}
									<div
										class="text-muted-foreground text-xs leading-relaxed pl-2 {classes.borderClass}"
									>
										{step.description}
									</div>
								{/if}
											...
											<div class="pl-2 mt-1 text-xs leading-relaxed">
												<div class="flex items-center gap-3 flex-wrap">
													...
													{#if index > 1}
														<a
															href={step.json_data ||
																'https://backend.cyberglobes.ai/api/v1/scripter/analyze-step-status'}
															target="_blank"
